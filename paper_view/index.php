...
elseif($flag==2 ){
	if(($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal')){
		$sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM paper_presentation s INNER JOIN Student st ON s.ID = st.ID WHERE UPPER(s.Title) = UPPER('$act')";
		$print ='Paper Presentation:-'.$act;
	}
	else if($_SESSION['login_flag'] == 2 && $_SESSION['Desg'] == 'HOD'){
		$sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM paper_presentation s INNER JOIN Student st ON s.ID = st.ID WHERE UPPER(s.Title) = UPPER('$act') AND st.Department ='$dept'";
		$print ='Paper Presentation:-'.$act.' '.$dept;
	}
	else if($_SESSION['login_flag'] == 2 && $_SESSION['Desg'] == 'Class Teacher'){
		$sql = "SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM paper_presentation s INNER JOIN Student st ON s.ID = st.ID WHERE UPPER(s.Title) = UPPER('$act') AND s.ClassID ='$cid'";
