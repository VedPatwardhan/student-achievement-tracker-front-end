...
$print = $_SESSION['print'];
	...
	if(empty($_POST['dept'])){
		...
		if(!empty($_POST['year']) && empty($_POST['division'])){	
			if($flag == 2 && $_SESSION['login_flag'] == 2){
				$sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM sports s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Year = '$year'";
			}
			elseif($flag == 1 && $_SESSION['login_flag'] == 2){
				$sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM sports s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Year = '$year'  AND $sql_search";
			 	$_SESSION['flag'] = 21;
			}	 
			...
			if($flag == 2 && $_SESSION['login_flag'] == 2){
			 	$sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM sports s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Class = '$div1' ";
			}
			elseif($flag == 1 && $_SESSION['login_flag'] == 2){
				$sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM sports s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Class = '$div1' AND $sql_search ";
				$_SESSION['flag'] = 21;
			}
    ...
    if ($statement->execute()) {
		...
		elseif($flag == 1 && (($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal'))){
				$sql="SELECT s.*,st.Rollno,st.Year,st.Full_Name FROM sports s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND $sql_search";
			$_SESSION['flag'] = 21;
		}	  
		  $_SESSION['sql_dept'] = " st.Department ='$dept' ";
