...
$div ='';
    ...
    if ($statement->execute()) {
        ...
        header("location:comp_cal.php?ID=".$id."&people=". urlencode(serialize($people))."&flag=".$flag."&sql1=".$sql1);
    }
     else{
	...
	elseif(!empty($_POST['dept']) && !empty($_POST['year']) && empty($_POST['division'])){
		...
		$year = mysqli_real_escape_string($link,$_REQUEST['year']);
		  $sql="SELECT s.*,st.Rollno,st.Year FROM competitive s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Year = '$year'";
	...
	elseif(!empty($_POST['dept']) && !empty($_POST['year']) && !empty($_POST['division'])){
		...
		$div1 = $year.$div;
		//echo $div1;
		  $sql="SELECT s.*,st.Rollno,st.Year FROM competitive s INNER JOIN Student st ON s.ID = st.ID WHERE st.Department ='$dept' AND st.Class = '$div1' ";
		  $sql1= " AND st.Department ='$dept' AND st.Year = '$year' ";
		   $statement = $connection->prepare($sql);
    if ($statement->execute()) {
        $people = $statement->fetchAll(PDO::FETCH_OBJ);
