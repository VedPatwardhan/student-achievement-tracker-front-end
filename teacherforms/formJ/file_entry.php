...
if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['publish'])   && isset($_POST['grant1']) && isset($_POST['license'])  && isset($_POST['amount']) && isset($_POST['amount'])   ) {
  ...
  $uid = uniqid("FJ-");
  $sql = 'INSERT INTO formj(UID,ID,Name,Title,Publish,Patent_Grant,License,Amount,Currentdate) VALUES(:uid,:id,:name,:title,:publish,:grant1,:license,:amount,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':title' => $title,':publish' => $publish,':grant1'=>$grant1,':license'=>$license,':amount'=>$amount,':currdate'=>$currdate])) {
