...
if (isset ($_POST['name'])  && isset($_POST['grantname']) && isset($_POST['authority'])   && isset($_POST['DO']) && isset($_POST['period'])  && isset($_POST['order']) && isset($_POST['amount'])   ) {
  ...
  $uid = uniqid("FG-");
  $sql = 'INSERT INTO formg(UID,ID,Name,Grantname,Authority,Period,OrderNo,Date_time,Amount,Currentdate) VALUES(:uid,:id,:name,:grantname,:authority,:period,:order,:dob,:amount,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':grantname' => $grantname,':authority' => $authority,':dob' => $dob,':period'=>$period,':order'=>$order,':amount'=>$amount,':currdate'=>$currdate])) {
