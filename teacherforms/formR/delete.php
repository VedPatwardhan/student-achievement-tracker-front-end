...
$sql = 'DELETE formr,files FROM formr INNER JOIN files  ON formr.UID = files.UID  WHERE formr.ID=:id AND formr.UID=:uid';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  header("Location:index.php");
	...
	$sql = 'DELETE FROM formr WHERE ID=:id and UID=:uid';
	$statement = $connection->prepare($sql);
	if ($statement->execute([':id' => $id,':uid' => $uid])) {
		header("Location: index.php");
	}
