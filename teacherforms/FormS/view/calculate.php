...
$print = isset($_SESSION['print']) ? $_SESSION ['print'] : 'FormS:- All Teachers';
$sql_search = '';
$sql_dept = '';
$_SESSION['flag'] = $flag;
  if($flag == 1){
...
<html lang="en">
  <head>
    <title>Form S: Information</title>
  <meta charset="utf-8">
      ...
      <?php } ?>
        ...
        <div class="collapse navbar-collapse" id="navbarTogglerDemo03">
          ...
          <input list="search" name="search" id="srch" onchange="myFunction(this)" class="input-field center" autocomplete="off" placeholder="Search By">
            ...
            <div id="FDP" style="display: none;">
              <input name="FDP" type="text" placeholder="Enter FDP" class="input-field">
              <button type="submit" class="form-button" ></span><i class="fa fa-search"></i>
              </button>
            </div>
            ...
            <div id="Topic" style="display: none;">
              <input name="Topic" type="text" placeholder="Enter Topic Authority" class="input-field">
              <button type="submit" class="form-button" ></span><i class="fa fa-search"></i>
              </button>
            </div> 
            <div id="Organizer" style="display: none;">
              <input name="Organizer" type="text" placeholder="Enter Organizer" class="input-field"><br>
              <button type="submit" class="form-button" ></span><i class="fa fa-search"></i>
              </button>
            </div> 
...
<script type="text/javascript">
  function myFunction(option){
          ...
          if(opt == "FDP"){
            document.getElementById("FDP").style.display = "block"; 
            document.getElementById("srch").style.display = "none";       
          }
          if(opt == "Level"){
      ...
      <table class="table table-bordered">
       <thead>
        ...
        <?php if($_SESSION['login_flag'] == 2 && isset($_SESSION['CH'])){  ?>
          ...
          <th>
            <form action="session_m.php" method="post">
            <input type="hidden" name="column" value="FDP">
            <input type="hidden" name="order" value="<?php echo $asc_or_desc?>">
            <input type="hidden" name="flag" value="<?php echo $flag ?>">
            <a href="session_m.php" onclick="this.closest('form').submit();return false;">FDP<i class="fas fa-sort<?php echo $column == 'FDP' ? '-' . $up_or_down : ''; ?>"></i></a>
          </form>
          </th>
          <th>
            <form action="session_m.php" method="post">
          ...
          <th>
            <form action="session_m.php" method="post">
            <input type="hidden" name="column" value="Topic">
            <input type="hidden" name="order" value="<?php echo $asc_or_desc?>">
            <input type="hidden" name="flag" value="<?php echo $flag ?>">
            <a href="session_m.php" onclick="this.closest('form').submit();return false;">Topic<i class="fas fa-sort<?php echo $column == 'Topic' ? '-' . $up_or_down : ''; ?>"></i></a>
            </form>
          </th>
          <th>
            <form action="session_m.php" method="post">
