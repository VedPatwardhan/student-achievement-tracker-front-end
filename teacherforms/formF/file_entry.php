...
if (isset ($_POST['name'])  && isset($_POST['sponsor']) && isset($_POST['support'])    && isset($_POST['grant1'])  && isset($_POST['year']) ) {
  ...
  $year=$_POST['year'];
  $uid = uniqid("FF-");
  $sql = 'INSERT INTO formf(UID,ID,Name,Sponsor,Support,Grant_money,Year,Currentdate) VALUES(:uid,:id,:name,:sponsor,:support,:grant1,:year,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':sponsor' => $sponsor,':support' => $support,':grant1'=>$grant1,':year'=>$year,':currdate'=>$currdate])) {
