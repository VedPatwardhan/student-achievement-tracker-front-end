...
if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['journal'])    && isset($_POST['level'])  && isset($_POST['volume']) && isset($_POST['page']) && isset($_POST['year']) && isset($_POST['isbn']) && isset($_POST['pub']) ) {
  ...
  $uid = uniqid("FL-");
  $sql = 'INSERT INTO forml(UID,ID,Name,Title,Journal,Level,Volume,Page,Year,Isbn,Publisher,Currentdate) VALUES(:uid,:id,:name,:title,:journal,:level,:volume,:page,:year,:isbn,:pub,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':title' => $title,':journal' => $journal,':level'=>$level,':volume'=>$volume,':page'=>$page,':year'=>$year,':isbn'=>$isbn,':pub'=>$pub,':currdate'=>$currdate])) {
