...
$sql = 'DELETE formn,files FROM formn INNER JOIN files  ON formn.UID = files.UID  WHERE formn.ID=:id AND formn.UID=:uid';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  header("Location:index.php");
	...
	$sql = 'DELETE FROM formn WHERE ID=:id and UID=:uid';
	$statement = $connection->prepare($sql);
	if ($statement->execute([':id' => $id,':uid' => $uid])) {
		header("Location: index.php");
	}
