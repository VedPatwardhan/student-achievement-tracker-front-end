...
if (isset($_POST['name']) && isset($_POST['collab']) && isset($_POST['DO'])    && isset($_POST['remark']) ) {
  ...
  $dob = date('Y-m-d',strtotime($rawdate));
   ...
   $sql = 'UPDATE formd SET Name=:name,Collab=:collab,Date_time=:dob,Remark=:remark WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name,':collab' => $collab,':dob' => $dob,':remark'=>$remark,':uid' => $uid])) {
