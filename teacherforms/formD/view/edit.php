...
$message = '';
$sql = 'SELECT * FROM formd WHERE UID=:uid';
...
if (isset($_POST['name']) && isset($_POST['collab']) && isset($_POST['DO'])   && isset($_POST['DE'])  && isset($_POST['remark']) ) {
  ...
  $de = date('Y-m-d',strtotime($rawdate1));
   ...
   $sql = 'UPDATE formd SET Name=:name,Collab=:collab,Date_start=:ds,Date_end=:de,Remark=:remark WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name,':collab' => $collab,':ds' => $ds,':de' => $de,':remark'=>$remark,':uid' => $uid])) {
      ...
      <form method="post">
        ...
        <div class="form-group">
         ...
         <div class="form-group">
          ...
          <input value="<?= $person->Collab; ?>" type="text" name="collab" id="collab" class="form-control">
        </div>
