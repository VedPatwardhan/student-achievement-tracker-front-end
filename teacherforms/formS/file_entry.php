...
if (isset ($_POST['name'])  && isset($_POST['fdp']) && isset($_POST['lvl'])   && isset($_POST['DO']) && isset($_POST['topic']) && isset($_POST['organizer'])   ) {
  ...
  $uid = uniqid("FS-");
  $sql = 'INSERT INTO forms(UID,ID,Name,FDP,Level,Topic,Date_time,Organizer,Currentdate) VALUES(:uid,:id,:name,:fdp,:lvl,:topic,:dob,:organizer,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':fdp' => $fdp,':lvl' => $lvl,':topic' => $topic,':dob' => $dob,':organizer'=>$organizer,':currdate'=>$currdate])) {
