...
if(isset($_SESSION["login_auth"]) || $_SESSION["login_auth"] == true || isset($_SESSION["login_admin"]) || $_SESSION["login_admin"] == true){
  if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['invest'])   &&  isset($_POST['remark']) ) {
    ...
    $remark=$_POST['remark'];
    $uid = uniqid("FE-");
    $sql = 'INSERT INTO forme(UID,ID,Name,Title,Investment,Remarks,Date_start) VALUES(:uid,:id,:name,:title,:invest,:remark,:ds)';
    $statement = $connection->prepare($sql);
    if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':title' => $title,':invest' => $invest,':remark'=>$remark,':ds'=>$currdate])) {
...
<div class="container">
  <div class="card mt-5">
    <div class="card-header">
      <h2>E.  Center  of innovation / excellence
</h2>
    </div>
    <div class="card-body">
      ...
      <?php endif; ?>
      <!-- <form method="post" action="/projects/teacherforms/FormE/file_entry.php"> -->
      <form method="post" action="create.php">
        ...
        <div class="form-group">
          ...
          <input type="text" name="invest" id="invest" class="form-control">
        </div>
