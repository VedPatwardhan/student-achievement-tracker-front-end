...
$sql = 'DELETE formk,files FROM formk INNER JOIN files  ON formk.UID = files.UID  WHERE formk.ID=:id AND formk.UID=:uid';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  header("Location:index.php");
	...
	$sql = 'DELETE FROM formk WHERE ID=:id and UID=:uid';
	$statement = $connection->prepare($sql);
	if ($statement->execute([':id' => $id,':uid' => $uid])) {
		header("Location: index.php");
	}
