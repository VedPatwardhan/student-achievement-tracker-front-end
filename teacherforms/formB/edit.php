...
if (isset ($_POST['activity'])  && isset($_POST['title']) && isset($_POST['speaker'])   && isset($_POST['DO']) && isset($_POST['participant'])  && isset($_POST['remark']) ) {
  ...
  $dob = date('Y-m-d',strtotime($rawdate));
   ...
   $sql = 'UPDATE formb SET Activity=:activity,Title=:title,Speaker=:speaker,Date_time=:dob,Participants=:participant,Remarks=:remark WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':activity' => $activity,':title' => $title,':speaker' => $speaker,':dob' => $dob,':participant'=>$participant,':remark'=>$remark,':uid' => $uid])) {
