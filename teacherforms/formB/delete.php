...
$sql = 'DELETE formb,files FROM formb INNER JOIN files  ON formb.UID = files.UID  WHERE formb.ID=:id AND formb.UID=:uid';
$dir_handle = '';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  $sql = 'DELETE FROM formb WHERE ID=:id and UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':id' => $id,':uid' => $uid])) {
    header("Location: index.php");
  }
