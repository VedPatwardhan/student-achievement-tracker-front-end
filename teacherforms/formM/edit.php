...
if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['faculty'])    && isset($_POST['sanction'])  && isset($_POST['fund']) && isset($_POST['duration']) && isset($_POST['amount']) && isset($_POST['major'])  ) {
  ...
  $major=$_POST['major'];
   ...
   $sql = 'UPDATE formm SET Name=:name,Faculty=:faculty,Title=:title,Sanction=:sanction,Fund=:fund,Duration=:duration,Amount=:amount,Major=:major WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name,':faculty' => $faculty,':title' => $title,':sanction' => $sanction,':fund'=>$fund,':duration'=>$duration,':amount'=>$amount,':major'=>$major,':uid' => $uid])) {
    ...
    <div class="card-body">
      ...
      <form method="post">
        ...
        <div class="form-group">
         ...
         <div class="form-group">
          <label for="duration">Duration of the Project/Scheme (Dates)
 </label>
