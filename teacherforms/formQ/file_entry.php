...
if (isset ($_POST['name'])  && isset($_POST['no']) && isset($_POST['dur'])   && isset($_POST['part']) && isset($_POST['prize'])  && isset($_POST['level']) ) {
  ...
  $uid = uniqid("FQ-");
  $sql = 'INSERT INTO formq(UID,ID,Name,Participants,Duration,Participant_name,Prize,Level,Currentdate) VALUES(:uid,:id,:name,:no,:dur,:part,:prize,:level,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':no' => $no,':dur' => $dur,':part' => $part,':prize'=>$prize,':level'=>$level,':currdate'=>$currdate])) {
