...
$sql = 'DELETE formq,files FROM formq INNER JOIN files  ON formq.UID = files.UID  WHERE formq.ID=:id AND formq.UID=:uid';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  header("Location:index.php");
	...
	$sql = 'DELETE FROM formq WHERE ID=:id and UID=:uid';
	$statement = $connection->prepare($sql);
	if ($statement->execute([':id' => $id,':uid' => $uid])) {
		header("Location: index.php");
	}
