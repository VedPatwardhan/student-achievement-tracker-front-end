...
if(empty($_POST['dept'])){
	...
	if(!empty($_POST['year']) && empty($_POST['division'])){	
		...
		elseif($flag == 1 && $_SESSION['login_flag'] == 2){
			$sql="SELECT * FROM student  WHERE Department ='$dept' AND Year = '$year'  AND $sql_search";
			 $_SESSION['flag'] = 21;
		}	 
		...
		elseif($flag == 1 && $_SESSION['login_flag'] == 2){
			$sql="SELECT * FROM student WHERE Department = '$dept' AND Class = '$div1' AND $sql_search ";
			$_SESSION['flag'] = 21;
		}
...
if ($statement->execute()) {
	...
	elseif($flag == 1 && (($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal'))){
			$sql="SELECT * FROM student WHERE Department ='$dept' AND $sql_search";
		$_SESSION['flag'] = 21;
	}	  
	  $_SESSION['sql_dept'] = " Department ='$dept' ";
...
elseif(!empty($_POST['dept']) && !empty($_POST['year']) && empty($_POST['division'])){
	...
	elseif($flag == 1 && (($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal'))){
	  ...
	  $_SESSION['sql_dept'] = " Department ='$dept' AND Year = '$year' ";
...
elseif(!empty($_POST['dept']) && !empty($_POST['year']) && !empty($_POST['division'])){
	...
	elseif($flag == 1 && (($_SESSION['login_flag'] == 3) || ($_SESSION['login_flag'] == 2 &&  $_SESSION['Desg'] == 'Principal'))){
	  ...
	  $_SESSION['sql_dept'] = " Department ='$dept' AND Year = '$year' AND Class = '$div1' ";
	  $_SESSION['print'] = $print.' || Div'.$div1;
	   $statement = $connection->prepare($sql);
if ($statement->execute()) {
	$people = $statement->fetchAll(PDO::FETCH_OBJ);
