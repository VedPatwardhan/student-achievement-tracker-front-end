...
if (isset ($_POST['Company']) && isset($_POST['Address']) && isset($_POST['Description']) && isset($_POST['Type'])&& isset($_POST['Job_Role']) && isset($_POST['Source']) 
  ...
  //echo $Com;
  $sql = 'INSERT INTO internship(UID,ID,Company,Address,Duration,Type,Description,Job_Role,Stipend,Source,Approve,Date_Join,Date_End) VALUES(:uid,:id,:Com,:address,:duration,:type,:descp,:job_role,:Stipend,
                                :source,:approve,:doj,:doe)';

  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':Com' => $Com,':address' => $address,':duration' => $duration,':type' => $type,':descp' => $descp,':job_role'=> $job_role,':Stipend'=> $Stipend,':source' => $source,':approve' => $approve,':doj' => $doj,':doe' => $doe])) {
