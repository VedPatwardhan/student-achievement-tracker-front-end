...
if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['pub'])   && isset($_POST['DO']) && isset($_POST['copy'])  && isset($_POST['isbn']) ) {
  ...
  $uid = uniqid("FK-");
  $sql = 'INSERT INTO formk(UID,ID,Name,Title,Publisher,Date_Pub,Copy,Link,Isbn,Currentdate) VALUES(:uid,:id,:name,:title,:pub,:dob,:copy,:link,:isbn,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':title' => $title,':pub' => $pub,':dob' => $dob,':copy'=>$copy,':link'=>$link,':isbn'=>$isbn,':currdate'=>$currdate])) {
