...
if (isset ($_POST['name'])  && isset($_POST['title']) && isset($_POST['pub'])   && isset($_POST['DO']) && isset($_POST['copy'])  && isset($_POST['isbn']) ) {
  ...
  $dob = date('Y-m-d',strtotime($rawdate));
   ...
   $sql = 'UPDATE formk SET Name=:name,Title=:title,Publisher=:pub,Date_Pub=:dob,Copy=:copy,Link=:link,Isbn=:isbn WHERE UID=:uid';
  $statement = $connection->prepare($sql);
      ...
      <form method="post">
        ...
        <div class="form-group">
         ...
         <div class="form-group">
          ...
          <input value="<?= $person->Date_Pub; ?>" type="date" name="DO" id="DO" class="form-control">
        </div>
         <div class="form-group">
