...
if (isset($_POST['title']) && isset($_POST['type']) && isset($_POST['organiser']) && isset($_POST['DO']) && isset($_POST['staff'])  && isset($_POST['sponsorship']) ) {
  ...
  $dob = date('Y-m-d',strtotime($rawdate));
   ...
   $sql = 'UPDATE formc SET Title=:title,Type=:type,Organiser=:organiser,Date_time=:dob,Staff=:staff,Sponsorship=:sponsorship WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':title' => $title,':type' => $type,':organiser' => $organiser,':dob' => $dob,':staff'=>$staff,':sponsorship'=>$sponsorship,':uid' => $uid])) {
