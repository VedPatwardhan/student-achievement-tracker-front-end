...
if (isset($_POST['title']) && isset($_POST['type']) && isset($_POST['organiser'])   && isset($_POST['DO']) && isset($_POST['staff'])  && isset($_POST['sponsorship']) ) {
  $type = $_POST['type'];
  $title = $_POST['title'];
  ...
  $uid = uniqid("FC-");
  $sql = 'INSERT INTO formc(UID,ID,Title,Type,Organiser,Date_time,Staff,Sponsorship,Currentdate) VALUES(:uid,:id,:title,:type,:organiser,:dob,:staff,:sponsorship,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':title' => $title,':type' => $type,':organiser' => $organiser,':dob' => $dob,':staff'=>$staff,':sponsorship'=>$sponsorship,':currdate'=>$currdate])) {
