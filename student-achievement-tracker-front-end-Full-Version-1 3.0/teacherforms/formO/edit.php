...
$message = '';
$sql = 'SELECT * FROM formo WHERE UID=:uid';
...
if (isset ($_POST['name'])  && isset($_POST['cls'])  && isset($_POST['achievement']) &&  isset($_POST['remark']) ) {
  ...
  $remark=$_POST['remark'];
   ...
   $sql = 'UPDATE formo SET Name=:name,Class=:cls,Achievement=:achievement,Remark=:remark WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name,':cls' => $cls,':achievement' => $achievement,':remark'=>$remark,':uid' => $uid])) {
