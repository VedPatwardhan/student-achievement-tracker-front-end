...
if (isset ($_POST['name'])  && isset($_POST['cls'])  && isset($_POST['achievement']) &&  isset($_POST['remark'])  ) {
  ...
  $remark=$_POST['remark'];
  $uid = uniqid("FO-");
  $sql = 'INSERT INTO formo(UID,ID,Name,Class,Achievement,Remark,Currentdate) VALUES(:uid,:id,:name,:cls,:achievement,:remark,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':cls' => $cls,':achievement' => $achievement,':remark'=>$remark,':currdate'=>$currdate])) {
