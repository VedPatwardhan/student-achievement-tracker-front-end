...
$sql = 'DELETE formi,files FROM formi INNER JOIN files  ON formi.UID = files.UID  WHERE formi.ID=:id AND formi.UID=:uid';
...
if ($statement->execute([':id' => $id,':uid' => $uid])) {
  ...
  header("Location:index.php");
	...
	$sql = 'DELETE FROM formi WHERE ID=:id and UID=:uid';
	$statement = $connection->prepare($sql);
	if ($statement->execute([':id' => $id,':uid' => $uid])) {
		header("Location: index.php");
	}
