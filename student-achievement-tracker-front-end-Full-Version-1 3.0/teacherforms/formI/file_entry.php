...
if (isset ($_POST['name'])  && isset($_POST['client']) && isset($_POST['title'])    && isset($_POST['amount'])  ) {
  ...
  $uid = uniqid("FI-");
  $sql = 'INSERT INTO formi(UID,ID,Name,Client,Title,Amount,Currentdate) VALUES(:uid,:id,:name,:client,:title,:amount,:currdate)';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':uid' => $uid,':id' => $id,':name' => $name,':client' => $client,':title' => $title,':amount'=>$amount,':currdate'=>$currdate])) {
