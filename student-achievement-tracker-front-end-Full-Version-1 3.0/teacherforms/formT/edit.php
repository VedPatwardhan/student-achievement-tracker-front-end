...
if (isset ($_POST['name'])  && isset($_POST['no']) && isset($_POST['dur'])   && isset($_POST['prize']) && isset($_POST['remark']) && isset($_POST['organizer'])   ) {
  ...
  $remark = $_POST['remark'];
   ...
   $sql = 'UPDATE formt SET Name=:name,Organizer=:organizer,Participants=:no,Duration=:dur,Prize=:prize,Remark=:remark WHERE UID=:uid';
  $statement = $connection->prepare($sql);
  if ($statement->execute([':name' => $name,':organizer'=>$organizer,':no' => $no,':dur' => $dur,':prize' => $prize,':remark' => $remark,'uid' => $uid])) {
